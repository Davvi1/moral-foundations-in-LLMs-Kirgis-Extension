...
Instruction-tuned only. Base models are a separate question and adding them is scope creep.

Core eight (run these first, they are the fallback if day two runs short):
Qwen2.5-0.5B / 1.5B / 3B / 7B-Instruct · Llama-3.2-1B / 3B-Instruct ·
Llama-3.1-8B-Instruct · Mistral-7B-Instruct-v0.3

Extension to ~13, added for statistical power, not coverage:
Qwen2.5-14B-Instruct · Gemma-2-2B-it · Gemma-2-9B-it · Phi-3-mini-4k-instruct ·
one further ≤9B instruct model of your choosing

**Verify every HuggingFace repo ID resolves before writing it into code.** These strings
are from recall and are exactly the kind of thing that is plausibly wrong.

Rationale for the extension: the pairwise rank statistic is computed over models, so n is
the number of models. At n = 8 the Spearman critical value at α = 0.05 sits near 0.74,
which means a 0.7 threshold is indistinguishable from no association. More models is the
cheap fix — a longer loop, no new code. Downloads dominate the added cost.

If day two runs short, drop to the core eight and report the reduced power explicitly
rather than quietly.
