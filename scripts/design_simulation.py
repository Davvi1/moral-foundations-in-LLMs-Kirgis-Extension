...
on R may be wide enough that the preregistered bands (0.25 / 1.0) cannot be resolved. This
must be settled BEFORE the confirmatory run, while N can still change, and before the
preregistration is deposited.
...
Two parts:
  ...
  2. SIMULATE   -- generate data under known R at several N, recover R by ANOVA method of
                   ...
                   the correct preregistered band.
...
# Preregistered bands, from state.md
...
def main() -> int:
    ...
    p("Generated by `scripts/design_simulation.py`. Decides the model roster size and whether "
      "the preregistered decision rule survives. **Run before the confirmatory data.**\n")
    ...
    # Method-specific residual SDs, per the preregistered structure:
    ...
    p("4. **No feasible N resolves an R that sits near a band boundary.** That is inherent to "
      "a hard-threshold rule, not a fixable power problem. The preregistration should therefore "
