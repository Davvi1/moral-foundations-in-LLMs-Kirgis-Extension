...
def test_run_label_produces_one_row_per_item(qwen_tok, prompt_cfg, items):
    ...
    assert all(r["condition"] == "label" for r in rows)
    assert rows[0]["n_options_found"] == 5
    assert rows[0]["score"] == pytest.approx(2.20, abs=1e-6)
    assert rows[0]["logprob_mass"] == pytest.approx(1.0, abs=1e-6)


def test_run_label_requests_logprobs_and_one_token(qwen_tok, prompt_cfg, items):
...
def test_run_label_handles_no_option_in_top_k(qwen_tok, prompt_cfg, items):
    """If the model puts no option token in the top-k, label scoring is undefined —
    it must flag parse_failed, not emit a fabricated score."""
    ps = _prompts(qwen_tok, prompt_cfg, items, 1)
    ids = C.option_token_ids(qwen_tok, prompt_cfg["options"])
    junk = qwen_tok.encode("Sorry", add_special_tokens=False)[0]
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {"text": "Sorry", "token_ids": [junk],
                                              "first_logprobs": {junk: -0.01}})
    rows = C.run_label(llm, SamplingParams, ps, ids)
    assert rows[0]["parse_failed"] is True
    assert rows[0]["n_options_found"] == 0
    assert math.isnan(rows[0]["score"])


def test_run_string_separates_options_0_and_1(qwen_tok, prompt_cfg, items):
    """The headline reason string scoring exists. Option 1 is made far more likely than
    option 0; a first-token readout could not distinguish them, full-sequence must."""
    ps = _prompts(qwen_tok, prompt_cfg, items, 1)
    base_n = len(qwen_tok.encode(ps[0].text, add_special_tokens=False))

    def responder(prompt, sp, tok):
        ids = tok.encode(prompt, add_special_tokens=False)
        opt = prompt[len(ps[0].text):]
        per_tok = -0.2 if opt == prompt_cfg["options"][1] else -5.0
        plp = [None] + [{tid: Logprob(logprob=(per_tok if i >= base_n else -0.5))}
                        for i, tid in enumerate(ids[1:], start=1)]
        return {"text": "", "prompt_logprobs_override": plp}

    llm = FakeLLM(qwen_tok, responder)
    rows = C.run_string(llm, SamplingParams, qwen_tok, ps, prompt_cfg["options"])
    assert len(rows) == 1
    assert rows[0]["n_options_found"] == 5
    assert rows[0]["token_boundary_clean"] is True
    assert rows[0]["score"] == pytest.approx(1.0, abs=0.15), (
        "option 1 was made dominant; the expectation should sit near 1")


def test_run_string_requests_prompt_logprobs(qwen_tok, prompt_cfg, items):
    ps = _prompts(qwen_tok, prompt_cfg, items, 1)
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {"text": ""})
    C.run_string(llm, SamplingParams, qwen_tok, ps, prompt_cfg["options"])
    _, sp = llm.calls[0]
    assert sp.prompt_logprobs == 0, "string scoring needs prompt_logprobs"
    assert sp.max_tokens == 1


def test_run_string_scores_five_continuations_per_item(qwen_tok, prompt_cfg, items):
    ps = _prompts(qwen_tok, prompt_cfg, items, 3)
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {"text": ""})
    C.run_string(llm, SamplingParams, qwen_tok, ps, prompt_cfg["options"])
    sent, _ = llm.calls[0]
    assert len(sent) == 3 * 5
...
def test_run_string_flags_dirty_token_boundary(qwen_tok, prompt_cfg, items):
    """Tokenizers are not always concatenative. If the option's tokens do not begin exactly
    where the prompt ended, the score is meaningless and must be flagged."""
    ps = _prompts(qwen_tok, prompt_cfg, items, 1)
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {"text": "",
                                              "prompt_logprobs_override": [None]})
    rows = C.run_string(llm, SamplingParams, qwen_tok, ps, prompt_cfg["options"])
    assert rows[0]["token_boundary_clean"] is False
    assert rows[0]["parse_failed"] is True


def test_run_string_records_both_normalisations(qwen_tok, prompt_cfg, items):
    ps = _prompts(qwen_tok, prompt_cfg, items, 1)
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {"text": ""})
    rows = C.run_string(llm, SamplingParams, qwen_tok, ps, prompt_cfg["options"])
    assert "score_alt_normalisation" in rows[0]
    assert not math.isnan(rows[0]["score_alt_normalisation"])
...
def test_all_conditions_agree_on_prompt_hash(qwen_tok, prompt_cfg, items):
    """The invariant, checked through the actual runners rather than by inspection."""
    ps = _prompts(qwen_tok, prompt_cfg, items, 2)
    ids = C.option_token_ids(qwen_tok, prompt_cfg["options"])
    llm = FakeLLM(qwen_tok, lambda p, sp, t: {
    ...
    rows = (C.run_label(llm, SamplingParams, ps, ids)
            ...
            + C.run_free(llm, SamplingParams, ps, greedy=False, k=2, seeds=[0, 1]))
    by_item: dict[int, set] = {}
    for r in rows:
        by_item.setdefault(r["item_id"], set()).add(r["prompt_sha"])
    for item_id, shas in by_item.items():
        assert len(shas) == 1, f"item {item_id} saw {len(shas)} different prompts"
